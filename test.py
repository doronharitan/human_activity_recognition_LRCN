...
def main():
    ...
    for i in range(len(images)):
        create_video_with_labels(save_path_videos, '{}_{}.avi'.format(label_decoder_dict[predicted_labels[i].item()], i),
                                 images[i], None, [predicted_labels[-1][i]], label_decoder_dict)
    predicted_labels, true_labels = torch.cat(predicted_labels), torch.tensor(true_labels)
