...
def split_data(ucf_list_root, seed, smaller_dataset=False):
    video_names_train, video_names_test, labels, labels_decoder_dict = get_video_list(ucf_list_root)
    if smaller_dataset:
...
def get_data(mode, video_names, list, labels=[]):
...
def get_video_list(ucf_list_root):  # get_list_of_video_names
    video_names_train, video_names_test, labels = [], [], []
    for file in os.listdir(ucf_list_root):
        if 'train' in file:
            with open(ucf_list_root + file) as f:
                video_names = f.readlines()
            video_names_train, labels = get_data('train', video_names, video_names_train, labels)
            ...
            labels_decoder_dict = {x.split(' ')[0] : x.split(' ')[1].rstrip('\n') for x in labels_decoder}
        else:
            with open(ucf_list_root + file) as f:
                video_names = f.readlines()
            video_names_test, _ = get_data('test', video_names, video_names_test)
